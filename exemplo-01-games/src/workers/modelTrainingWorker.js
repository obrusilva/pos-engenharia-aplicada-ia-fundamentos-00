...
function makeContext(products, users) {
    ...
    const platformsIndex = Object.fromEntries(
    ...
    return {
        ...
        // price + age + genres(one-hot) + platforms(one-hot)
        dimentions: 2 + genres.length + platforms.length
    }
}
...
function encodeProduct(product, context) {
    ...
    const genre = oneHotWeighted(
        context.genresIndex[product.genre],
        context.numGenres,
        WEIGHTS.genre
    )

    const platform = oneHotWeighted(
        context.platformsIndex[product.platform],
        context.numPlatforms,
        WEIGHTS.platform
    )
...
function encodeUser(user, context) {
    ...
    return tf.concat1d(
        [
            ...
            tf.zeros([context.numGenres]), // genre ignorada,
            tf.zeros([context.numPlatforms]), // platform ignorada,
